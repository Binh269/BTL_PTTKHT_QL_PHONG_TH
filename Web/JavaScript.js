$(document).ready(function () {
    ...
    function loadHongTable() {
        ...
        $.post('API.aspx', { action: 'thietbi_hong' }, function (data) {
            container.empty();
            if (data.length > 0) {
    ...
    function loadDangDungTable() {
        ...
        $.post('API.aspx', { action: 'thietbi_dangdung' }, function (data) {
            container.empty();
            if (data.length > 0) {
                var groupedData = groupBy(data, 'maphong'); 
                for (var room in groupedData) {
    ...
    function loadKhongDungTable() {
        ...
        $.post('API.aspx', { action: 'thietbi_khongdung' }, function (data) {
            container.empty();
            if (data.length > 0) {
                var groupedData = groupBy(data, 'maphong'); 
                for (var room in groupedData) {
